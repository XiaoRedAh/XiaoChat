...
管道中添加`StringDecoder`：自动的把接收的Bytebuf数据转（解码为）String，不需要手动处理
管道中添加`StringEncoder`：自动的把待发送的Bytebuf数据转（编码为）String，不需要手动处理
...
```java
/**
 * 自定义对象编码器，继承自MessageToByteEncoder，将待发送的字节转换为指定对象
 * @author XiaoRed
 * @date 2023/11/30 13:39
 */
...
public class MyChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    protected void initChannel(SocketChannel channel) throws Exception {
        channel.pipeline()
                ...
                .addLast(new ObjEncoder(MsgInfo.class)) //将待发送的字节转化为MsgInfo对象
                .addLast(new MyClientHandler());
    }
}
```
