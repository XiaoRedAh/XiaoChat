...
step2：服务端收到数据后，首先是将消息**使用多线程进行异步入库**【**使用ThreadPoolTaskExecutor开一个线程进行入库处理**】留存。接下来判断好友是否在线，只有好友在线，才能通过好友ID从服务端的 Map 结构中获取到通信渠道 Channel
...
**为什么需要断线重连机制**
使用类似微信这样的聊天软件，有时候会由于网络环境问题而导致没有网可用，也就是暂时性的断网，此时发送信息，就不能被接收到，但是当网络恢复后，聊天就又可以继续了。
这就需要断线重连手段，来保证在网络，或其他原因导致断线后，可以在网络恢复后，通信也随之恢复（如果没有这样的功能，那么就需要退出登录再重新连接）

**实现思路**
使用**ScheduledExecutorService**执行定时任务，**定时监测连接状态**，来判断是否保持良好的链接，如果断开，则进行重连恢复通信。
当网络连接断开以后，客户端会重新启动Netty客户端，拿到新的channel，通过这个新的channel向服务端发送重新链接的请求。这个发起链接的过程，和系统最开始登录时的链接有所区别。
恢复单聊：断线重连需要将用户的ID信息一同发送给服务端，让服务端去更新用户与通信管道`Channel`的绑定关系。
恢复群聊：还需要更新群组`ChannelGroup`，把用户重连后，对应的新channel加入群组映射中。

## 协议定义

org/chat/server/protocol/login/ReconnectRequest.java
断线重连请求协议：这里的设计得比较简单，只包括了必须有的用户ID。
实际的业务场景还会包含：重连时间、重连设备、网络环境，以及当前对话的内容暂存和恢复数据传输聊天记录等等操作。以保证恢复通信后可以保证数据传输的衔接。
```java
/**
 * 断线重连请求协议
 * @author XiaoRed
...
最后，记得往Command中添加协议对应的指令码
还要记得把协议和指令码的对应关系保存到Packet的map中

## 服务端处理请求

服务端处理断线重连请求
* 恢复单聊：更新channel与uid映射
* 恢复群聊：更新uid对应群组的ChannelGroup中的映射

org/chat/server/socket/handler/ReconnectHandler.java
```java
/**
 * 服务端处理断线重连请求
 * @author XiaoRed
...
org/chat/client/Application.java
使用ScheduledExecutorService定时任务进行连接状态检查，发现断线，则发送断线重连请求
```java
...
***
**测试**
开两个客户端，可以正常通信。把Netty服务端直接关闭，两个客户端不能通信。将Netty服务端重启后，又可以通信了（两个客户端不用重新登录），证明断线重连配置成功
